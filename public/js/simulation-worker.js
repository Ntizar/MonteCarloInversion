...
function createRNG(seed) {
  ...
  return {
    ...
    randInt(max) { return Math.floor(uniform() * max); },
  };
}

function mean(arr) { let s = 0; for (let i = 0; i < arr.length; i++) s += arr[i]; return s / arr.length; }
function std(arr, ddof = 1) { const m = mean(arr); let s = 0; for (let i = 0; i < arr.length; i++) s += (arr[i] - m) ** 2; return Math.sqrt(s / (arr.length - ddof)); }
function variance(arr, ddof = 1) { const m = mean(arr); let s = 0; for (let i = 0; i < arr.length; i++) s += (arr[i] - m) ** 2; return s / (arr.length - ddof); }
...
function rollingVariance(arr, window) {
  const result = [];
  for (let i = window; i <= arr.length; i++) { const s = arr.slice(i - window, i); result.push(variance(s, 1)); }
  return result;
}
function correlation(a, b) {
  ...
  let cov = 0, sa = 0, sb = 0;
  for (let i = 0; i < n; i++) { const da = a[i] - ma, db = b[i] - mb; cov += da * db; sa += da * da; sb += db * db; }
  const denom = Math.sqrt(sa * sb);
  return denom > 0 ? cov / denom : 0;
}
function clamp(v, lo, hi) { return Math.min(hi, Math.max(lo, v)); }
function averageOrZero(arr) { return arr.length > 0 ? mean(arr) : 0; }
function computeLogReturnsFromPrices(prices) {
  const r = [];
